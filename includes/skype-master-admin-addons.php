<?php
if( is_multisite() ) {
	function menu_multi_skype_admin_addons(){
	// Create menu
	add_submenu_page( 'skype-master', 'Add-ons', 'Add-ons', 'manage_options', 'skype-master-admin-addons', 'skype_master_admin_addons' );
	}
}
else {
	// Create menu
	function menu_single_skype_admin_addons(){
		if ( is_admin() )
		add_submenu_page( 'skype-master', 'Add-ons', 'Add-ons', 'manage_options', 'skype-master-admin-addons', 'skype_master_admin_addons' );
	}
}

function skype_master_admin_addons(){
?>
<div class="wrap">
...
<h2><b>&nbsp;<?php echo get_option('skype_master_name'); ?> Add-ons</b></h2>
<?php
if(!class_exists('skype_master_admin_addons_table')){
	require_once( dirname( __FILE__ ) . '/skype-master-admin-addons-table.php');
}
//Prepare Table of elements
$wp_list_table = new skype_master_admin_addons_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'network_admin_menu', 'menu_multi_skype_admin_addons' );
}
else {
add_action( 'admin_menu', 'menu_single_skype_admin_addons' );
}